...
void TankOdometry::update(int32_t fl_ticks, int32_t fr_ticks,
    ...
    // Global frame transformation (tank drive moves only forward/backward, no strafe)
...
void TankOdometry::calculateDelta(int32_t dfl, int32_t dfr, int32_t drl, int32_t drr,
    ...
    // Left side average (front-left + rear-left) / 2
    // Right side average (front-right + rear-right) / 2
    float left_avg_mm = (dfl_mm + drl_mm) / 2.0f;
    float right_avg_mm = (dfr_mm + drr_mm) / 2.0f;
    
    // Forward motion is average of both sides
    d_forward_mm = (left_avg_mm + right_avg_mm) / 2.0f;
    
    // Rotation is difference between sides divided by track width
    // Positive rotation = counter-clockwise (right side moves forward more than left)
